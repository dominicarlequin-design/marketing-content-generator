...
5. **Git protocol.** Conventional Commits, one concern per commit. **Never commit secrets or
   ...
   gets pushed and is visible to the whole team. main is protected — no direct pushes for anyone,
   including admins. Every change lands via a PR with at least 1 teammate approval. This is
   intended to be enforced by GitHub branch protection once configured; until then, treat it as a
   firm team norm. Work on a feature branch, open a PR, get a nod, then merge.
